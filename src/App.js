...
function App() {
  // State variables for Back to Top button visibility and scroll progress bar width
  const [showTopBtn, setShowTopBtn] = useState(false);
  const [scrollPercent, setScrollPercent] = useState(0);

  // Effect to handle scroll events for Back to Top button and scroll bar progress
  useEffect(() => {
    const handleScroll = () => {
      setShowTopBtn(window.scrollY > 200); // Show button after 200px scroll

      const scrollTop = window.scrollY;
      const docHeight = document.documentElement.scrollHeight - window.innerHeight;
      const percent = docHeight > 0 ? (scrollTop / docHeight) * 100 : 0;
      setScrollPercent(percent); // Update scroll progress percentage
    };
  ...
  return (
    <>
      ...
      {/* Scroll Progress Bar styled based on scrollPercent state */}
      ...
      <main>
        ...
        <section id="hero" className="hero-section">
          ...
          <div>
            ...
            <a href="/View Insights" className="hero-btn" target="_blank" rel="noopener noreferrer">📘 Learn More</a>
          </div>
